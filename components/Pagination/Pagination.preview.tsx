...
import type { PaginationInteractionState, PaginationNumberItem, PaginationSize, PaginationType } from './Pagination.types';

const TYPES: PaginationType[] = ['arrows', 'numbers', 'buttons'];
const SIZES: PaginationSize[] = ['md', 'sm'];
const INTERACTION_STATES: PaginationInteractionState[] = ['default', 'hover', 'focus', 'disabled'];
...
function buildNumberItems(activePage: string): PaginationNumberItem[] {
  return [
...
export default function PaginationPreviewPage() {
  const [type, setType] = useState<PaginationType>('numbers');
  const [size, setSize] = useState<PaginationSize>('md');
  const [interactionState, setInteractionState] = useState<PaginationInteractionState>('default');
      ...
      >
        ...
        >
          <label style={{ display: 'grid', gap: spacing.scale['4'] }}>
            ...
            <select
              value={type}
              onChange={(event) => setType(event.target.value as PaginationType)}
              style={{
          ...
          <label style={{ display: 'grid', gap: spacing.scale['4'] }}>
            ...
            <select
              value={size}
              onChange={(event) => setSize(event.target.value as PaginationSize)}
              style={{
                minHeight: spacing.scale['40'],
                borderStyle: 'solid',
                borderWidth: border.width['1'],
          ...
          <label style={{ display: 'grid', gap: spacing.scale['4'] }}>
            ...
            <select
              ...
              onChange={(event) => setInteractionState(event.target.value as PaginationInteractionState)}
              style={{
                minHeight: spacing.scale['40'],
                borderStyle: 'solid',
                borderWidth: border.width['1'],
